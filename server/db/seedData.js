...
// A randomly-generated fallback password must still satisfy validPassword()
// below - otherwise an account seeded with this fallback would be created
// successfully but could never actually sign in, for the same reason a
// non-compliant operator-supplied password can't (see seedSecret).
function randomCompliantPassword() {
  return `${randomBytes(24).toString("hex")}Aa1!`;
}

// Reads a required secret from the environment. For passwords specifically,
// also enforces the same complexity policy the login route checks
// (validPassword) whenever an operator has explicitly set a value. Without
// this, e.g. SEED_ADMIN_PASSWORD could be set to something that gets seeded
// and bcrypt-hashed without error, but the login route would then reject it
// before ever reaching the database - silently producing an admin account
// nobody can sign into, with no error pointing at the real cause.
function seedSecret(name, fallback, { isPassword = true } = {}) {
  const value = process.env[name];
  if (process.env.NODE_ENV === "production" && !value) {
    throw new Error(`${name} must be set in production.`);
  }
  if (isPassword && value && !validPassword(value)) {
    throw new Error(
      `${name} does not meet the login password policy and would create an ` +
      `account that can never sign in. ${PASSWORD_REQUIREMENTS}`
    );
  }
  return value || fallback || randomCompliantPassword();
}
...
const e2eFacultyPassword = e2eEnabled ? seedSecret("E2E_FACULTY_PASSWORD", "") : randomCompliantPassword();
const e2eAdminPassword = e2eEnabled ? seedSecret("E2E_ADMIN_PASSWORD", "") : randomCompliantPassword();
...
const teacherCodes = [
  ...new Set([
    ...
    ...seedData.schedules.flatMap((item) => extractTeacherCodes(item.teacher).map((code) => code.toUpperCase()))
  ])
].sort();
...
seedData.teachers = teacherCodes.map((code) => {
  ...
  if (overridePassword && !validPassword(overridePassword)) {
    throw new Error(
      `FACULTY_${code}_PASSWORD does not meet the login password policy and ` +
      `would create an account that can never sign in. ${PASSWORD_REQUIREMENTS}`
    );
  }
  const sharedFacultyPassword = process.env.SEED_FACULTY_PASSWORD;
  if (sharedFacultyPassword && !validPassword(sharedFacultyPassword)) {
    throw new Error(
      `SEED_FACULTY_PASSWORD does not meet the login password policy and ` +
      `would create accounts that can never sign in. ${PASSWORD_REQUIREMENTS}`
    );
  }
  const defaultPassword = overridePassword || randomCompliantPassword();
...
// Local/staging department directory. Each department receives a temporary HOD
// from its existing faculty roster so an administrator can correct assignments
// from the Department Center without creating duplicate accounts.
const seededDepartmentNames = [
  "Administration",
...
const usedTemporaryHodIds = new Set();
const facultyForDepartment = (departmentName, index) => {
  const exact = seedData.teachers.filter((teacher) => teacher.department === departmentName && !usedTemporaryHodIds.has(teacher.id));
  const fallback = seedData.teachers.find((teacher) => !usedTemporaryHodIds.has(teacher.id));
  const hod = exact[0] || fallback || seedData.teachers[index % seedData.teachers.length];
  if (hod) usedTemporaryHodIds.add(hod.id);
  return hod;
};
...
// uses these stable keys, allowing operators to replace every local account
// without changing application code. Empty values are intentionally ignored.
function accountEnvPrefix(account) {
  const encodedId = String(account.id).split("").map((character) => /[a-z0-9]/i.test(character) ? character.toUpperCase() : `_X${character.charCodeAt(0).toString(16).toUpperCase()}X`).join("");
  return `ACCOUNT_${encodedId}`;
}
function applyAccountCredentialOverrides(collection) {
  for (const account of collection) {
    const prefix = accountEnvPrefix(account);
    const email = process.env[`${prefix}_EMAIL`];
    const password = process.env[`${prefix}_PASSWORD`];
    if (email) account.email = String(email).trim().toLowerCase();
    if (password) {
      if (!validPassword(password)) throw new Error(`${prefix}_PASSWORD does not meet the login password policy. ${PASSWORD_REQUIREMENTS}`);
      account.password = bcrypt.hashSync(String(password), 12);
      account.passwordVersion = 0;
    }
  }
}
applyAccountCredentialOverrides(seedData.admins);
applyAccountCredentialOverrides(seedData.students);
applyAccountCredentialOverrides(seedData.teachers);
