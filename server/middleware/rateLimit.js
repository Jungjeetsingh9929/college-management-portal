...
// Key builders live in their own module so they can be unit-tested without
// importing the database layer (and therefore `pg`).
import { bodyFieldKey } from "./rateLimitKeys.js";
...
// `scope` controls how wide a single bucket is. Without it the bucket key
// included req.path, so a limiter mounted on a whole prefix (app.use("/api",
// ...)) was really a *per-endpoint* limiter: the "120 requests/minute across
// the API" limiter actually allowed 120/minute to /api/students, another 120
// to /api/subjects, and so on. Pass an explicit scope string for prefix-wide
// limiters; leave it unset for route-level ones, where per-path is correct.
export function rateLimit({ windowMs, limit, message, keyGenerator, scope, backoffBaseMs = 0, backoffMaxMs = 0 }) {
  return (req, res, next) => {
    const identity = keyGenerator ? keyGenerator(req) : clientKey(req);
    const key = scope ? `${scope}:${identity}` : `${req.baseUrl || ""}:${req.path}:${identity}`;
