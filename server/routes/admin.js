...
function resolveHod(db, value) {
  const hodId = String(value || "");
  if (!hodId) return null;
  const hod = (db.teachers || []).find((item) => item.id === hodId);
  if (!hod) throw new Error("HOD must be an existing faculty account.");
  return hod;
}
...
adminRouter.post("/access-requests/:id/approve", requireAuth, requireAdmin, async (req, res) => {
  ...
  // Each branch only checked its own collection, and with a case-sensitive
  // compare - so the same address could be approved once as a student and
  // again as a teacher, or slip through on a different capitalisation.
  if (emailInUse(db, request.email)) return res.status(409).json({ message: "An account already exists for this email." });
  if (role === "student") {
    db.students ||= [];
    if (request.rollNumber && (db.students || []).some((item) => String(item.rollNumber || "").trim().toLowerCase() === String(request.rollNumber).trim().toLowerCase())) return res.status(409).json({ message: "This roll number is already assigned to another student." });
    db.students.push({ id: crypto.randomUUID(), name: request.name, rollNumber: request.rollNumber || `REQ-${Date.now()}`, className: request.className || "", department: request.department || "", email: normalizeEmail(request.email), password: request.password, passwordVersion: 0, phone: request.phone || "", guardian: request.guardian || "", graduationYear: request.graduationYear || "", approvalStatus: "approved", approvedAt: new Date().toISOString() });
  } else {
    db.teachers ||= [];
    let code = `USR${String(request.id).replace(/[^a-z0-9]/gi, "").slice(-6).toUpperCase()}`;
    // The code is derived from the request id, which is not guaranteed
    // unique across collections; a collision would silently merge two
    // teachers\' timetables, since every access check keys off teacher code.
    while (db.teachers.some((item) => String(item.code || "").toUpperCase() === code)) code = `USR${crypto.randomUUID().replace(/[^a-z0-9]/gi, "").slice(0, 6).toUpperCase()}`;
    db.teachers.push({ id: crypto.randomUUID(), code, name: request.name, department: request.department || "", email: normalizeEmail(request.email), password: request.password, passwordVersion: 0, role: "teacher", phone: request.phone || "", cabin: "", subjects: [] });
  }
...
adminRouter.post("/departments", requireAuth, requireAdmin, async (req, res) => { try { validateKeys(req.body || {}, ["name", "hodId"]); const name = requiredText(req.body.name, "Department", { max: 120 }); const db = await readDb(); db.departments ||= []; if (db.departments.some((item) => item.name.toLowerCase() === name.toLowerCase())) return res.status(409).json({ message: "Department already exists." }); const hod = resolveHod(db, req.body.hodId); const department = { id: crypto.randomUUID(), name, hodId: hod?.id || "" }; db.departments.push(department); if (hod) hod.department = name; await writeDb(db); res.status(201).json({ department }); } catch (error) { res.status(400).json({ message: error.message === "HOD must be an existing faculty account." ? error.message : "Invalid department data." }); } });
adminRouter.put("/departments/:id", requireAuth, requireAdmin, async (req, res) => { try { validateKeys(req.body || {}, ["name", "hodId"]); const db = await readDb(); db.departments ||= []; const department = db.departments.find((item) => item.id === req.params.id); if (!department) return res.status(404).json({ message: "Department not found." }); const name = requiredText(req.body.name, "Department", { max: 120 }); if (db.departments.some((item) => item.id !== department.id && item.name.toLowerCase() === name.toLowerCase())) return res.status(409).json({ message: "Another department already uses this name." }); const hod = resolveHod(db, req.body.hodId); const previousName = department.name; const previousHodId = department.hodId; const previousOwner = db.departments.find((item) => item.id !== department.id && item.hodId === hod?.id); if (previousOwner) previousOwner.hodId = ""; department.name = name; department.hodId = hod?.id || ""; department.temporaryHod = false; if (hod) hod.department = department.name; if (previousName !== department.name) { (db.students || []).forEach((student) => { if (student.department === previousName) student.department = department.name; }); (db.teachers || []).forEach((teacher) => { if (teacher.department === previousName || teacher.id === previousHodId) teacher.department = department.name; }); } await writeDb(db); res.json({ department }); } catch (error) { res.status(400).json({ message: error.message === "HOD must be an existing faculty account." ? error.message : "Invalid department data." }); } });
