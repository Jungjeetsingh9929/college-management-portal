...
## Public origin configuration (`CLIENT_ORIGIN` / `FRONTEND_URL`)

This used to be a silent-failure surface: a wrong or unset value baked an
unreachable host into every attendance QR code, which still rendered and
still scanned and only failed on the student's phone, where nobody on the
staff side would ever see it. Two changes close that:

- **Attendance QR codes no longer depend on it.** `POST /attendance/sessions`
  and friends now return an origin-less `qrPath`; the teacher's browser
  resolves it against `window.location.origin` before encoding the QR image.
  The scanned link therefore always points at the deployment the teacher is
  demonstrably able to reach.
- **The server validates the value at boot.** In production it refuses to
  start if `CLIENT_ORIGIN`/`FRONTEND_URL` is missing, unparseable, carries a
  path, or resolves to loopback — so a bad value is caught by the deploy,
  not by a classroom. Plain `http` logs a warning. Outside production nothing
  is ever fatal.

Still required before release: set `CLIENT_ORIGIN` to the exact public origin
(e.g. `https://portal.example.edu`, no trailing path). Password-reset and fee
reminder email links still come from this value and have no browser-side
