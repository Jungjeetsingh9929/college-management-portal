...
try {
  ...
  const resetRequest = await json("/auth/request-password-reset", {
    method: "POST",
    body: JSON.stringify({ email: "student001@example.edu" })
  });
  assert.equal(resetRequest.ok, true);
  assert.ok(resetRequest.resetToken, "NODE_ENV=test should return the reset token directly instead of emailing it");

  const unknownEmailReset = await json("/auth/request-password-reset", {
    method: "POST",
    body: JSON.stringify({ email: "no-such-account@example.edu" })
  });
  assert.equal(unknownEmailReset.ok, true);
  assert.equal(
    unknownEmailReset.resetToken,
    undefined,
    "requesting a reset for a nonexistent account must not reveal a token"
  );

  const badToken = await request("/auth/reset-password", {
    method: "POST",
    body: JSON.stringify({ token: "not-a-real-token", newPassword: "Another-rotated-password-2026!" })
  });
  assert.equal(badToken.response.status, 400);

  const finalPassword = "Reset-via-email-flow-2026!";
  const resetOutcome = await json("/auth/reset-password", {
    method: "POST",
    body: JSON.stringify({ token: resetRequest.resetToken, newPassword: finalPassword })
  });
  assert.equal(resetOutcome.ok, true);

  const reusedToken = await request("/auth/reset-password", {
    method: "POST",
    body: JSON.stringify({ token: resetRequest.resetToken, newPassword: "Yet-another-password-2026!" })
  });
  assert.equal(reusedToken.response.status, 400, "a reset token must not be usable twice");

  const loginAfterReset = await json("/auth/login", {
    method: "POST",
    body: JSON.stringify({ email: "student001@example.edu", password: finalPassword, role: "student" })
  });
  assert.equal(loginAfterReset.user.role, "student");
