  ...
  return (
    <div className="page-stack student-dashboard">
      ...
      <section className="student-information-grid">
        ...
        <div className="panel"><div className="section-heading"><div><span className="eyebrow">Academics</span><h2>Performance</h2></div><GraduationCap size={18} /></div>{portal.academics.sgpa != null || portal.academics.cgpa != null || portal.academics.subjects?.length ? <div className="performance-summary"><div><span>SGPA{portal.academics.semester ? ` · Sem ${portal.academics.semester}` : ""}</span><strong>{portal.academics.sgpa != null ? Number(portal.academics.sgpa).toFixed(2) : "—"}</strong></div><div><span>CGPA</span><strong>{portal.academics.cgpa != null ? Number(portal.academics.cgpa).toFixed(2) : "—"}</strong></div><div className="performance-subjects">{portal.academics.subjects.map((item) => <div key={item.subject || item.code}><span>{item.subject || item.code}</span><strong>{item.grade ? `${item.grade} · ${item.marks}` : item.marks ?? "—"}</strong></div>)}</div><Link className="secondary-button" to="/my-results">View all results</Link></div> : <EmptyState title="Academic results not published" text="Internal marks, semester results, SGPA, and CGPA will appear here." />}</div>
        ...
        <div className="panel"><div className="section-heading"><div><span className="eyebrow">Learning resources</span><h2>Notes</h2></div><FileText size={18} /></div>{notes.length ? <div className="list-stack">{notes.map((note) => <div className="list-row" key={note.id}><div><strong>{note.title}</strong><span>{note.teacherName} · {note.createdAt ? new Date(note.createdAt).toLocaleDateString() : "Recently"}</span></div><button type="button" className="ghost-button" onClick={() => downloadToFile(`/shared/notes/${note.id}/file`, note.file?.name || note.title)}>Download</button></div>)}</div> : <EmptyState title="No notes yet" text="Lecture notes and resources shared by your faculty will appear here." />}</div>
      </section>
