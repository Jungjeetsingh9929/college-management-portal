          ...
          <form className="admin-form" onSubmit={saveSubject}>
            {Object.keys(blankSubject).map((field) => (
              <label key={field}>
                {subjectLabels[field]}
                {field === "semester" ? (
                  <select value={subjectForm.semester} onChange={(e) => setSubjectForm({ ...subjectForm, semester: e.target.value })}>
            ...
            <table>
              ...
              <tbody>
                {subjects.map((subject) => (
                  <tr key={subject.id}>
                    <td>{subject.subjectName}<span>{subject.code} · {subject.className}</span><span>{subject.semester ? `Semester ${subject.semester}` : "Semester not set"} · {Number.isFinite(subject.credits) ? `${subject.credits} credit${subject.credits === 1 ? "" : "s"}` : "Credits not set"}</span></td>
