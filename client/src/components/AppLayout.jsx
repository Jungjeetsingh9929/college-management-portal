...
export function AppLayout({ children }) {
  ...
  const linksByRole = {
    ...
    student: [
      ...
      { to: "/complaints", label: "Complaints", icon: ClipboardList }, { to: "/student-records", label: "My Student Record", icon: UsersRound },
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="topbar-actions"><GlobalSearch /><QuizNotifications /><NotificationCenter /><button className="profile-chip" type="button" onClick={() => navigate("/account")}><span className="avatar">{user?.name?.slice(0, 1).toUpperCase()}</span><span className="profile-copy"><strong>{user?.name}</strong><small>{user?.role}</small></span></button></div>
        </header>
